...
该服务处理 `privateMaterialApi` 投递的私人 PDF：下载原件、定位题号、按相邻题号纵向裁剪、全文 OCR、上传题图，并把“题号 + OCR 文字 + 原题图”回写到私人题库。服务同时提供小程序排版页使用的 PDF/Word 文档导出接口。
...
- OCR 文字用于题目总览、搜索和知识点初筛，低置信度或有墨迹风险时标记为待核对；
- 裁剪原图始终保留并作为编辑、PDF/Word 排版的可信来源；
...
`POST /export` 接收小程序排版设置与题组，生成真实 PDF 或 DOCX。支持 A4/A5、单栏/双栏、字号、题间距、答题留白、答案行内/文末，以及试题和答案解析分成两个文件。题目存在裁剪图时优先使用原图，否则使用文字题干。
